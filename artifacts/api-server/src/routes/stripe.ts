...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const subscription =
        await stripeStorage.getActiveSubscriptionByCustomerId(
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { priceId, plan, billing } = req.body as {
        priceId?: string;
        plan?: string;
        billing?: string;
      };
      ...
      if (!rawId.startsWith("price_")) {
        const interval =
          billing === "annual" || billing === "yearly" ? "year" : "month";
        const rows = await stripeStorage.listProductsWithPrices();
        ...
        if (!match) {
          res.status(400).json({
            error: `No ${interval}ly price found for plan "${rawId}". Make sure Stripe products are seeded.`,
          });
          return;
        }
