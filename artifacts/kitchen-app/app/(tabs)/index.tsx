...
function formatRelativeTime(isoString: string): string {
  ...
  return diffH < 24
    ? `${diffH}h ago`
    : sent.toLocaleDateString("en-AU", { day: "numeric", month: "short" });
}
...
export default function TodayScreen() {
  ...
  const nextFn =
    sortedFunctions.find((f) => timeToMinutes(f.startTime) > nowMinutes) ??
    sortedFunctions.find((f) => timeToMinutes(f.endTime) > nowMinutes) ??
    null;
  const nextFnMins = nextFn
    ? timeToMinutes(nextFn.startTime) - nowMinutes
    : null;
  ...
  const showBroadcast =
    broadcastMessage !== null && broadcastMessage.id !== dismissedBroadcastId;
  ...
  function handleClearBroadcast() {
    Alert.alert(
      "Remove alert?",
      "This will remove the message for all staff.",
      [
  ...
  function renderCountdown() {
    ...
    const urgentColor =
      nextFnMins <= 30 ? "#EF4444" : nextFnMins <= 60 ? AMBER : colors.primary;
    ...
    return (
      <GlassCard style={s.countdownCard} accentColor={urgentColor}>
        <Pressable
          ...
          onPress={() => {
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
            router.push(`/function/${nextFn.id}`);
          }}
        >
          <LinearGradient
            ...
            <View style={s.countdownMinRow}>
              <Text style={[s.countdownNum, { color: urgentColor }]}>
                {nextFnMins <= 0
                  ? "NOW"
                  : nextFnMins >= 60
                    ? `${Math.floor(nextFnMins / 60)}h ${nextFnMins % 60}m`
                    : nextFnMins}
              </Text>
              {nextFnMins > 0 && nextFnMins < 60 && (
                <Text style={[s.countdownUnit, { color: urgentColor }]}>
                  min
                </Text>
              )}
            </View>
            <Text style={s.countdownName} numberOfLines={1}>
              {nextFn.name}
            </Text>
            <Text style={s.countdownMeta}>
              {nextFn.room} · {nextFn.floor} · {nextFn.guestCount} guests ·{" "}
              {nextFn.startTime}–{nextFn.endTime}
            </Text>
          </View>
        </Pressable>
      </GlassCard>
        ...
        <View style={s.myCardHeader}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[s.myRole, { color: rc }]}>
              {currentMember.role} · Shift {currentMember.shiftStart}–
              {currentMember.shiftEnd}
            </Text>
          </View>
          <View
            style={[
          ...
          >
            <Ionicons
              ...
              size={12}
              color={
                notificationsEnabled ? colors.accent : colors.mutedForeground
              }
            />
            <Text
              style={[
            ...
            <View style={s.myEvents}>
              {myFunctions.map((fn) => {
                ...
                return (
                  ...
                  >
                    <Text style={[s.myEventTime, { color: rc }]}>
                      {fn.startTime}
                    </Text>
                    <View style={{ flex: 1 }}>
                      <Text style={s.myEventName} numberOfLines={1}>
                        {fn.name}
                      </Text>
                      <Text style={s.myEventSub}>
                        {fn.room} · {fn.guestCount} guests
                      </Text>
                    </View>
                    <View
                      style={[s.fnTypePill, { backgroundColor: tc + "22" }]}
                    >
                      <Text style={[s.fnTypeText, { color: tc }]}>
                        {fn.functionType}
                      </Text>
                    </View>
                    <Feather
  ...
  function renderFunctionList() {
    ...
    return (
      <>
        ...
        >
          <Text
            style={[s.sectionLabel, { marginHorizontal: 0, marginBottom: 0 }]}
          >
            Today's Functions
          </Text>
          {activeFnsCount > 0 && (
        ...
        {sortedFunctions.length === 0 && (
          ...
          >
            ...
            >
              Add functions from the Functions tab
            </Text>
          </View>
        )}
        {isManager
          ? sortedFunctions.map((fn, idx) => {
              ...
              const totalDietary = dietaryReqs.reduce(
                (sum, d) => sum + d.count,
                0,
              );
              const nextCourse =
                fn.serviceEvents && fn.serviceEvents.length > 0
                  ? (fn.serviceEvents.find(
                      (e) => timeToMinutes(e.time) > nowMinutes,
                    ) ?? null)
                  : fn.serviceTimes
                    ? ((
                        [
                        ...
                        .filter(Boolean)
                        .find((c) => timeToMinutes(c!.time) > nowMinutes) ??
                      null)
                    : null;
              const fnMins = timeToMinutes(fn.startTime) - nowMinutes;
              const isUrgent = fnMins > 0 && fnMins <= 30;
              const isActive =
                fnMins <= 0 && timeToMinutes(fn.endTime) > nowMinutes;
              ...
              return (
                ...
                >
                  ...
                  <View style={s.fnCardTop}>
                    ...
                    >
                      <Text style={[s.fnTimeBadgeNum, { color: statusColor }]}>
                        #{idx + 1}
                      </Text>
                      <Text style={[s.fnTimeBadgeText, { color: statusColor }]}>
                        {fn.startTime}
                      </Text>
                      <Text
                        style={[
                          s.fnTimeBadgeEnd,
                          { color: statusColor + "90" },
                        ]}
                      >
                        {fn.endTime}
                      </Text>
                      <Text
                        style={[s.fnTimeBadgeType, { color: statusColor }]}
                        numberOfLines={2}
                      >
                        {abbreviateType(fn.functionType)}
                      </Text>
                    </View>
                    <View style={s.fnBody}>
                      <View
                      ...
                      >
                        <Text
                          style={[s.fnName, { flex: 1, marginRight: 8 }]}
                          numberOfLines={1}
                        >
                          {fn.name}
                        </Text>
                      ...
                      {teamProgress.map(({ team, done, total, pct }) => {
                        ...
                        return (
                          <View key={team} style={s.fnSectionRow}>
                            <Text style={[s.fnSectionLabel, { color: tc2 }]}>
                              {team
                                .replace(" Kitchen", " K.")
                                .replace(" Larder", " L.")
                                .replace(" Team", " T.")}
                      ...
                      {nextCourse && (
                        ...
                        >
                          ...
                          >
                            {nextCourse.label}: {nextCourse.time}
                          </Text>
                        </View>
                      )}
                    </View>
                  ...
                  {(totalDietary > 0 ||
                    hasSevere ||
                    fnAlerts.some((a) => a.severity !== "info")) && (
                    <View style={s.fnAlertRow}>
                      {fnAlerts
                        ...
                        .map((a) => {
                          ...
                          > = {
                            dietary: "alert-triangle",
                            overlap: "copy",
                            staffing: "users",
                            timeline: "clock",
                            prep: "check-square",
                          };
                          const OPS_LABELS: Record<AlertCategory, string> = {
                            dietary: "Allergen risk",
                            overlap: "Room conflict",
                            staffing: "Understaffed",
                            timeline: "Timeline late",
                            prep: "Prep behind",
                          };
                          return (
                            <View
                            ...
                            >
                              <Feather
                                name={OPS_ICONS[a.category]}
                                size={10}
                                color={ac}
                              />
                              <Text style={[s.fnAlertText, { color: ac }]}>
                                {OPS_LABELS[a.category]}
                              </Text>
                            </View>
                          );
                        })}
                      {hasSevere && (
                        ...
                        >
                          <Ionicons
                            ...
                            color="#EF4444"
                          />
                          <Text style={[s.fnAlertText, { color: "#EF4444" }]}>
                            Severe allergen
                          </Text>
                        </View>
                      )}
                      {dietaryReqs.slice(0, 3).map((d, i) => {
                        ...
                        return (
                          ...
                          >
                            <Text style={[s.fnAlertText, { color: dc }]}>
                              {d.count}× {d.name}
                            </Text>
                          </View>
                        );
                  ...
                  <View style={s.fnCardTop}>
                    ...
                    <View style={s.fnBody}>
                      ...
                      <View style={s.fnMetaRow}>
                        ...
                        <View style={s.fnMetaChip}>
                          <Ionicons
                            ...
                            color={colors.mutedForeground}
                          />
                          <Text style={s.fnMetaChipText}>{fn.floor}</Text>
                        </View>
                        <View style={s.fnMetaChip}>
                          <Ionicons
                            name="people"
                      ...
                      {fnAlerts
                        .filter(
                          (a) =>
                            a.severity !== "info" && a.category !== "dietary",
                        )
                        .slice(0, 1)
                        .map((a) => {
                          const ac =
        ...
        >
          ...
          <Text
            style={{
              fontSize: 12,
              fontFamily: "Inter_700Bold",
              color: prepPercent >= 1 ? colors.accent : colors.primary,
            }}
          >
      ...
      >
        ...
        >
          {entries.map(([name, { count, color }]) => (
            ...
            >
              <Text
                style={{ fontSize: 12, fontFamily: "Inter_700Bold", color }}
              >
                {count}×
              </Text>
              <Text
  ...
  const broadcastBanner =
    showBroadcast && broadcastMessage ? (
      <View
        style={[
          s.broadcastBanner,
          { backgroundColor: AMBER + "12", borderColor: AMBER + "50" },
        ]}
      >
        <View style={s.broadcastTop}>
          <View
            style={[s.broadcastIconWrap, { backgroundColor: AMBER + "25" }]}
          >
            <Ionicons name="megaphone" size={16} color={AMBER} />
          </View>
          <View style={s.broadcastBody}>
            <Text style={[s.broadcastLabel, { color: AMBER }]}>
              Message to all staff
            </Text>
            <Text style={s.broadcastText}>{broadcastMessage.text}</Text>
            <Text style={s.broadcastMeta}>
              From: {broadcastMessage.senderName} ·{" "}
              {broadcastMessage.senderRole} ·{" "}
              {formatRelativeTime(broadcastMessage.sentAt)}
            </Text>
          </View>
        </View>
        <View style={[s.broadcastActions, { borderTopColor: AMBER + "30" }]}>
          {isManager ? (
            <>
              <Pressable
                style={({ pressed }) => [
                  s.broadcastBtn,
                  {
                    ...
                    borderRightWidth: 1,
                    borderRightColor: AMBER + "30",
                  },
                ]}
                onPress={openCompose}
              >
                <Text style={[s.broadcastBtnText, { color: colors.info }]}>
                  Edit
                </Text>
              </Pressable>
              <Pressable
              ...
              >
                <Text style={[s.broadcastBtnText, { color: "#EF4444" }]}>
                  Remove alert
                </Text>
              </Pressable>
            </>
            ...
            >
              <Text
                style={[s.broadcastBtnText, { color: colors.mutedForeground }]}
              >
                Got it
              </Text>
            </Pressable>
              ...
              <View style={s.sheet}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {currentMember && (
                      <Text style={s.sheetSub}>
                        From: {currentMember.name} · {currentMember.role}
                      </Text>
                    )}
                  </View>
                  <Pressable
                ...
                <Pressable
                  style={({ pressed }) => [
                    ...
                    {
                      backgroundColor:
                        draftText.trim().length > 0 ? AMBER : colors.secondary,
                ...
                <View style={s.tipRow}>
                  ...
                  <Text style={s.tipText}>
                    Staff will see this message when they open the app.
                  </Text>
                </View>
              </View>
            </TouchableWithoutFeedback>
        ...
        <ScrollView style={{ flex: 0.58 }} showsVerticalScrollIndicator={false}>
          <View style={s.header}>
            <View style={s.headerLeft}>
              ...
              <Text style={[s.headerTitle, { fontSize: 30 }]}>
                Today's Service
              </Text>
            </View>
            {isManager && (
              ...
              >
                <Ionicons
                  name="megaphone"
                  size={18}
                  color={showBroadcast ? AMBER : colors.mutedForeground}
                />
              </Pressable>
            )}
          </View>
    ...
    >
          ...
          <View style={ap.headerBadges}>
            {criticalCount > 0 && (
              <View style={[ap.sevBadge, { backgroundColor: "#EF444420" }]}>
                <Text style={[ap.sevBadgeText, { color: "#EF4444" }]}>
            ...
            {warningCount > 0 && (
              <View style={[ap.sevBadge, { backgroundColor: "#F59E0B20" }]}>
                <Text style={[ap.sevBadgeText, { color: "#F59E0B" }]}>
            ...
            {infoCount > 0 && (
              <View style={[ap.sevBadge, { backgroundColor: "#3B82F620" }]}>
                <Text style={[ap.sevBadgeText, { color: "#3B82F6" }]}>
                  {infoCount} info
                </Text>
              </View>
            )}
      ...
      {expanded &&
        alerts.map((alert, i) => {
          ...
          const isLast =
            i === alerts.length - 1 && !alerts.some((a) => a.dismissible);
          return (
            <Pressable
              key={alert.id}
              style={[
            ...
            >
              ...
              <View style={ap.rowBody}>
                <Text style={[ap.rowTitle, { color: colors.foreground }]}>
                  {alert.title}
                </Text>
                <Text
