...
function getTeamDescription(team: PrepTeam): string {
  switch (team) {
    ...
    case "Hot Kitchen":
      return "Hot mains, hot sauces, firing & cooking";
    case "Pastry":
...
export default function PrepScreen() {
  ...
  const { functions, prepItems, staff, currentStaffId, togglePrepItem } =
    useKitchen();
  ...
  const activeFunctionIds = useMemo(
    () => new Set(activeFunctions.map((f) => f.id)),
    [activeFunctions],
  );
    ...
    return (
      ...
      >
        <View
          style={[
            s.teamColorBar,
            { backgroundColor: tc, opacity: item.completed ? 0.3 : 1 },
          ]}
        />
        <View
          style={[
          ...
          <View style={s.itemTopRow}>
            ...
            >
              <Feather
                name="clock"
                size={10}
                color={item.completed ? colors.accent : "#EF4444"}
              />
              <Text
                style={[
                  s.deadlineText,
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={s.header}>
          <View style={s.headerLeft}>
            ...
            <Text style={s.subtitle}>
              By team — with leader, tasks & deadlines
            </Text>
          </View>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            {printFnId && (
              <Pressable
                ...
                onPress={() => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                  router.push(`/prep-print/${printFnId}`);
                }}
              >
                <Feather
                  name="printer"
        ...
        {!canToggle && currentStaffId && (
          ...
          >
            ...
            <Text style={[s.accessBadgeText, { color: "#8B5CF6" }]}>
              View only — only managers and team leaders can check off prep
              tasks
            </Text>
          </View>
        )}
        {!currentStaffId && (
          ...
          >
            ...
            <Text
              style={[s.accessBadgeText, { color: colors.mutedForeground }]}
            >
              ...
              <Text style={s.progressLabel}>
                {selectedDate === "today"
                  ...
                  : selectedDate === "tomorrow"
                    ? "Tomorrow's prep progress"
                    : "Day-after prep progress"}
              </Text>
            ...
            <View style={s.progressBar}>
              {percent === 1 ? (
                ...
                <LinearGradient
                  ...
                  style={[s.progressFill, { width: `${percent * 100}%` }]}
                />
              )}
            </View>
          </View>
        ...
        {filteredItems.length === 0 ? (
          <View style={s.emptyBox}>
            ...
            <Text style={s.emptySubtitle}>
              There are no functions or prep tasks scheduled for{" "}
              {DATE_TABS.find((t) => t.key === selectedDate)?.short}.
            </Text>
          </View>
        ) : (
          <>
                ...
                {TEAM_ORDER.map((team) => {
                  ...
                  return (
                    <View
                      ...
                      style={[
                        s.teamSection,
                        isTablet && { width: "50%", paddingHorizontal: 4 },
                      ]}
                    >
                      <GlassCard
                        style={[s.teamHeaderCard, { borderColor: tc + "40" }]}
                        accentColor={tc}
                      >
                        <View
                          style={[
                        ...
                        >
                          <View
                            style={[s.teamColorBlock, { backgroundColor: tc }]}
                          />
                          <View style={s.teamCardInfo}>
                            <Text style={[s.teamName, { color: tc }]}>
                              {team}
                            </Text>
                            <Text style={s.teamDesc}>
                          ...
                          >
                            <Text
                              style={[
                                s.teamBadgeText,
                                { color: allDone ? colors.accent : tc },
                              ]}
                            >
                              {done}/{items.length}
                            </Text>
                        ...
                        {leader && (
                          ...
                          >
                            ...
                            <View style={s.leaderInfo}>
                              <Text style={[s.leaderLabel, { color: tc }]}>
                                Chef in Charge / Team Leader
                              </Text>
                              <Text
                                style={[
                                  s.leaderName,
                        ...
                        {members.length > 0 && (
                          ...
                          >
                            ...
                            {members.map((m) => (
                              ...
                              >
                                <Text style={[s.memberPillText, { color: tc }]}>
                                  {m.name.split(" ")[0]}
                                </Text>
                              </View>
                            ))}
                      ...
                      {items.map((item) =>
                        renderItem(item, activeFunctions.length > 1),
                      )}
                    </View>
                  );
                })}
            ...
            {viewMode === "by-event" && (
              ...
              >
                {byEvent.map(({ fn, teamMap }) => {
                  if (!fn) return null;
                  const fnItems = filteredItems.filter(
                    (p) => p.functionId === fn.id,
                  );
                  ...
                  return (
                    ...
                    >
                      <View style={s.eventHeader}>
                        <View style={s.eventTimePill}>
                          <Text style={s.eventTimePillText}>
                            {fn.startTime}
                          </Text>
                        </View>
                        <View style={{ flex: 1 }}>
                          ...
                          <Text style={s.eventRoom}>
                            Room: {fn.room} · {fn.guestCount} guests · {fnDone}/
                            {fnItems.length} done
                          </Text>
                        </View>
                        <Pressable
                        ...
                        return (
                          <View key={team}>
                            <View style={s.subTeamHeader}>
                              <View
                                style={[s.subTeamDot, { backgroundColor: tc }]}
                              />
                              <Text style={[s.subTeamLabel, { color: tc }]}>
                                {team}
                              </Text>
                              {leader && (
                                <Text style={s.subLeaderText}>
                                  {leader.name}
                                </Text>
                              )}
                              <Text style={s.subTeamCount}>
                                {" "}
                                · {done}/{items.length}
                              </Text>
                            </View>
