...
export default function AnalyticsScreen() {
  ...
  const { functions, prepItems, staff, sickStaffIds, currentStaffId } =
    useKitchen();
  ...
  const totalGuests = useMemo(
    () => functions.reduce((n, f) => n + f.guestCount, 0),
    [functions],
  );
  const prepDone = useMemo(
    () => prepItems.filter((p) => p.completed).length,
    [prepItems],
  );
  ...
  const onTimeRate =
    functions.length > 0
      ? Math.round((onTimeCount / functions.length) * 100)
      : 100;
  ...
  const incidents = useMemo(() => {
    const result = [];
    const nutCount = functions.reduce(
      (n, f) =>
        n +
        f.dietaryRequirements
          .filter((d) => d.name.toLowerCase().includes("nut"))
    ...
    if (nutCount > 0)
      result.push({
        emoji: "🥜",
        title: "Nut Allergy Checks",
        desc: `${nutCount} flagged across functions`,
        icon: "✓",
        iconColor: "#22C55E",
      });
    if (sickStaffIds.length > 0)
      result.push({
        emoji: "🤒",
        title: "Staff Sick Calls",
        desc: `${sickStaffIds.length} staff called in sick`,
        icon: "⚠",
        iconColor: "#EAB308",
      });
    ...
    if (overdue.length > 0)
      result.push({
        emoji: "⏰",
        title: "Pending Prep Tasks",
        desc: `${overdue.length} tasks not yet complete`,
        icon: "⚠",
        iconColor: "#F97316",
      });
    return result;
  ...
  const laborPerFn =
    functions.length > 0 ? (laborHours / functions.length).toFixed(1) : "0";
  ...
  if (!isManager) {
    return (
      <View style={[s.root, s.noAccess]}>
        ...
        >
          Analytics is for managers only.{"\n"}Sign in as yourself on the Roster
          tab.
        </Text>
        <Pressable
          style={({ pressed }) => ({
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={s.kpiGrid}>
          <View style={s.kpiCard}>
            <View style={s.kpiRow}>
              <Text style={[s.kpiValue, { color: "#F0F6FC" }]}>
                {functions.length}
              </Text>
          ...
          <View style={s.kpiCard}>
            <View style={s.kpiRow}>
              ...
              <Text
                style={[
                  s.kpiTrend,
                  { color: prepPct > 80 ? "#22C55E" : "#EF4444" },
                ]}
              >
                {prepDone}/{prepTotal}
              </Text>
                ...
                <View style={s.barWrapper}>
                  <View
                    style={{
                      ...
                      backgroundColor:
                        d.pct === 0 ? "rgba(255,255,255,0.04)" : "#F97316",
                      borderRadius: 4,
                      ...(d.today
                        ? {
                            shadowColor: "#F97316",
        ...
        <Text
          style={[s.sectionTitle, { marginHorizontal: 20, marginBottom: 12 }]}
        >
          Function Performance
        </Text>
        <View style={s.fnListCard}>
          ...
          {functions.map((fn, i) => {
            ...
            return (
              <View
                key={fn.id}
                style={[s.fnItem, isLast && { borderBottomWidth: 0 }]}
              >
                      ...
                      <View
                        style={[
                          s.progressFill,
                          {
                            width: `${pct}%`,
                            backgroundColor:
                              pct === 100 ? "#22C55E" : "#F97316",
                          },
                        ]}
                      />
                    </View>
                    <Text style={s.progressText}>
                      {pct === 100 ? "100% done" : `${pct}%`}
                    </Text>
                  </>
                )}
              </View>
          ...
          <>
            ...
            {incidents.map((inc, i) => (
              <View
                ...
                style={[
                  s.incidentRow,
                  i === incidents.length - 1 && { borderBottomWidth: 0 },
                ]}
              >
                ...
                <Text style={{ fontSize: 14, color: inc.iconColor }}>
                  {inc.icon}
                </Text>
              </View>
            ))}
        ...
        <View style={s.laborCard}>
          ...
          <Text style={s.laborBench}>
            Staff on today: {staff.length} · Sick: {sickStaffIds.length}
          </Text>
          <Text style={s.laborWarn}>
            {staff.length} staff across {functions.length} function
