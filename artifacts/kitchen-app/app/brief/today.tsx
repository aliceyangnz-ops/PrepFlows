...
export default function TodayBriefScreen() {
  ...
  const onShiftStaff = useMemo(
    () =>
      staff.filter(
        (m) => allOnShiftIds.has(m.id) && !sickStaffIds.includes(m.id),
      ),
  ...
  const totalDietary = useMemo(
    () =>
      functions.reduce(
        (n, fn) =>
          n + (fn.dietaryRequirements?.reduce((a, r) => a + r.count, 0) ?? 0),
        0,
      ),
    [functions],
  );
    ...
    <View style={s.root}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.heroSub}>
          {dayLabel} · {functions.length} function
          {functions.length !== 1 ? "s" : ""} · {onShiftStaff.length} staff on
        </Text>
        ...
        <View style={s.statsStrip}>
          ...
          <View style={s.statBox}>
            <Text style={s.statNum}>
              {functions.reduce((n, fn) => n + fn.guestCount, 0)}
            </Text>
              ...
              <View key={member.id} style={s.nameCard}>
                <View style={s.nameRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      {isLeader && (
                        ...
                        >
                          <Text style={[s.leaderBadgeText, { color: tc }]}>
                            TEAM LEADER
                          </Text>
                        </View>
                      )}
                    ...
                    <Text style={s.roleText}>
                      {member.role}
                      {member.staffNumber ? ` · ${member.staffNumber}` : ""}
                    </Text>
                    {fns.length > 0 && (
                      <Text
                        style={{
                  ...
                  {member.section && (
                    <View style={[s.teamPill, { backgroundColor: tc + "18" }]}>
                      <Text style={[s.teamPillText, { color: tc }]}>
                        {member.section}
                      </Text>
                    </View>
                  )}
                ...
                {isLeader && member.phone && (
                  ...
                  >
                    ...
                    >
                      {member.phone} · Call for questions
                    </Text>
                  </View>
                )}
              </View>
                ...
                <View style={s.fnMeta}>
                  ...
                  {fn.floor ? (
                    <>
                      ...
                      <Text style={s.fnMetaText}>{fn.floor}</Text>
                    </>
                  ) : null}
                  <Text style={s.fnMetaDot}>·</Text>
                  <Feather name="clock" size={12} color="#94A3B8" />
                  <Text style={s.fnMetaText}>
              ...
              {sections.map((team) => {
                ...
                return (
                  <View key={team}>
                    ...
                    {leader && (
                      <View
                        style={[s.memberRow, { backgroundColor: tc + "08" }]}
                      >
                        ...
                        <Text style={[s.memberRole, { color: tc }]}>
                          Leader
                        </Text>
                        {leader.phone && (
                          <Text
                    ...
                    {others.map((m, idx) => (
                      <View
                        ...
                        style={[
                          s.memberRow,
                          idx === others.length - 1 &&
                            !sections[sections.indexOf(team) + 1] && {
                              borderBottomWidth: 0,
                            },
                        ]}
                      >
          ...
          <View style={[s.dietaryCard, { marginBottom: 14 }]}>
            <View style={s.dietaryHeader}>
              ...
              <Text style={s.dietaryHeaderText}>
                ⚠️ Dietary across all functions — check every plate
              </Text>
            </View>
            {functions.flatMap((fn) =>
              (fn.dietaryRequirements ?? []).map((req, i) => (
                <View key={`${fn.id}-${i}`} style={s.dietaryRow}>
                  <Text style={s.dietaryFnName} numberOfLines={1}>
                    {fn.room}
                  </Text>
