...
function getCategoryStyle(category: string): {
  ...
  switch (category) {
    ...
    default:
      return { color: "#6B7A94", icon: "circle", label: "", prominent: false };
  }
}
...
export default function FunctionDetailScreen() {
        ...
        serviceEvents: fn.serviceEvents
          ...
          : fn.serviceTimes
            ? (["amuse", "entree", "main", "dessert", "supper"] as const)
                ...
                .map((k) => ({
                  time: fn.serviceTimes![k]!,
                  label: k.charAt(0).toUpperCase() + k.slice(1),
                }))
            : [],
        dietaryRequirements: fn.dietaryRequirements
          ? fn.dietaryRequirements.map((d) => ({ ...d }))
          : [],
  ...
  const tc = getFunctionTypeColor(
    editing ? draft.functionType : fn.functionType,
  );
  ...
  const statusColor =
    status === "LIVE" ? "#EAB308" : status === "Done" ? "#22C55E" : "#3B82F6";
  const minsToEnd = Math.max(
    0,
  ...
  const timeChanged =
    ...
    JSON.stringify(fn.serviceEvents ?? []) !==
      JSON.stringify(draft.serviceEvents);
  ...
  function removeMenuItem(idx: number) {
    setDraft((d) => {
      if (!d) return d;
      return { ...d, menu: d.menu.filter((_, i) => i !== idx) };
    });
    setHasUnsaved(true);
  }
    ...
    setDraft((d) => {
      ...
      return {
        ...
        dietaryRequirements: [
          ...
          { name: "New Requirement", count: 1, note: "" },
        ],
      };
    });
    setHasUnsaved(true);
  ...
  function removeDietaryReq(idx: number) {
    setDraft((d) => {
      ...
      return {
        ...d,
        dietaryRequirements: d.dietaryRequirements.filter((_, i) => i !== idx),
      };
    });
    setHasUnsaved(true);
  }
  ...
  function handleSave() {
    ...
    updateFunction(fn.id, {
      ...
      dietaryRequirements: draft.dietaryRequirements.filter(
        (d) => d.count > 0 && d.name.trim(),
      ),
    ...
    Alert.alert(
      "Saved",
      "Function updated — all team members will see the changes.",
      [{ text: "OK" }],
    );
  }
        ...
        { text: "Keep editing", style: "cancel" },
        {
          ...
          onPress: () => {
            setDraft({
              ...
              room: snap.room,
              floor: snap.floor,
              functionType: snap.functionType,
              startTime: snap.startTime,
              endTime: snap.endTime,
              guestCount: String(snap.guestCount),
              ...
              serviceEvents: snap.serviceEvents
                ...
                : snap.serviceTimes
                  ? (["amuse", "entree", "main", "dessert", "supper"] as const)
                      ...
                      .map((k) => ({
                        time: snap.serviceTimes![k]!,
                        label: k.charAt(0).toUpperCase() + k.slice(1),
                      }))
                  : [],
              dietaryRequirements: snap.dietaryRequirements
                ? snap.dietaryRequirements.map((d) => ({ ...d }))
                : [],
  ...
  function parseMenuLine(line: string): {
    course: string;
    name: string;
    desc: string;
    tags: string[];
  ...
  const activeCourses: Array<{ label: string; time: string }> =
  ...
  const s = StyleSheet.create({
    ...
    dietaryEditCountLabel: {
      fontSize: 12,
      fontFamily: "Inter_500Medium",
      color: colors.mutedForeground,
    },
    dietaryEditCountInput: {
      width: 60,
      fontSize: 16,
      fontFamily: "Inter_700Bold",
      color: colors.foreground,
      ...
      >
        ...
        <View style={s.hero}>
          ...
          <View
            style={[
              s.typePill,
              { backgroundColor: tc + "15", borderColor: tc + "40" },
            ]}
          >
            <Text style={[s.typeText, { color: tc }]}>
              {editing ? draft.functionType : fn.functionType}
            </Text>
          </View>
          <Text style={s.eventName}>
            {editing ? draft.name || fn.name : fn.name}
          </Text>
          <View style={s.roomRow}>
            <Text style={s.roomLabel}>Room </Text>
            <Text style={s.roomValue}>
              {editing ? draft.room || fn.room : fn.room}
            </Text>
            <View style={s.floorTag}>
              <Text style={s.floorTagText}>
                · {editing ? draft.floor || fn.floor : fn.floor}
              </Text>
            </View>
          </View>
          {!editing && status === "LIVE" && (
            ...
            minsToStart <= 240 && (
              ...
              >
                ...
                >
                  {"Starting in "}
                  {minsToStart >= 60
                    ? `${Math.floor(minsToStart / 60)}h ${minsToStart % 60}m`
                    : `${minsToStart}m`}
                </Text>
              </View>
            )}
          <View style={s.infoGrid}>
            <View style={s.infoBox}>
              <Text style={s.infoNum}>
                {editing ? draft.guestCount : fn.guestCount}
              </Text>
              <Text style={s.infoLabel}>Covers</Text>
            </View>
            <View style={s.infoBox}>
              <Text style={[s.infoNum, { color: "#F97316" }]}>
                {editing ? draft.startTime : fn.startTime}
              </Text>
              <Text style={s.infoLabel}>Service</Text>
            </View>
            <View style={s.infoBox}>
              <Text style={s.infoNum}>
                {editing ? draft.endTime : fn.endTime}
              </Text>
              <Text style={s.infoLabel}>End</Text>
            </View>
            <View style={s.infoBox}>
              <Text
                style={[s.infoNum, { color: colors.primary, fontSize: 13 }]}
                numberOfLines={1}
              >
                {editing
                  ? draft.functionType.split(" ")[0]
                  : fn.functionType.split(" ")[0]}
              </Text>
        ...
        {!editing && timeChanged && (
          ...
          >
            ...
            <Text style={[s.bannerText, { color: "#F59E0B" }]}>
              Times have been updated — check the run sheet below
            </Text>
          </View>
        )}
        ...
        {editing && (
          <>
            ...
            <View style={[s.editCard, { borderColor: colors.primary + "60" }]}>
              ...
              >
                <Feather name="edit-2" size={14} color={colors.primary} />
                <Text style={[s.editCardTitle, { color: colors.primary }]}>
                  Function Details
                </Text>
                <Text
                  style={{
              ...
              <View style={s.fieldRow}>
                ...
                <TextInput
                  style={s.fieldInput}
                  value={draft.room}
                  onChangeText={(v) => updateDraft("room", v)}
                  placeholder={fn.room}
                  placeholderTextColor={colors.mutedForeground}
                />
              </View>
              <View style={s.fieldRow}>
                ...
                <TextInput
                  style={s.fieldInput}
                  value={draft.floor}
                  onChangeText={(v) => updateDraft("floor", v)}
                  placeholder={fn.floor}
                  placeholderTextColor={colors.mutedForeground}
                />
              </View>
              <View style={s.fieldRow}>
                ...
                <TextInput
                  ...
                  onChangeText={(v) => updateDraft("guestCount", v)}
                  keyboardType="number-pad"
                  placeholder={String(fn.guestCount)}
                  placeholderTextColor={colors.mutedForeground}
                />
              </View>
              <View style={s.fieldRow}>
                ...
                <TextInput
                  ...
                  onChangeText={(v) => updateDraft("startTime", v)}
                  placeholder="HH:MM"
                  placeholderTextColor={colors.mutedForeground}
                />
              </View>
              <View style={s.fieldRow}>
                ...
                <TextInput
                  style={s.fieldInput}
                  value={draft.endTime}
                  onChangeText={(v) => updateDraft("endTime", v)}
                  placeholder="HH:MM"
                  placeholderTextColor={colors.mutedForeground}
                />
              </View>
              <View style={s.fieldRow}>
                ...
                <TextInput
                  style={s.fieldInput}
                  value={draft.chefInCharge}
                  onChangeText={(v) => updateDraft("chefInCharge", v)}
                  placeholder="e.g. Marco Russo"
                  placeholderTextColor={colors.mutedForeground}
                />
              </View>
                  ...
                  {FUNCTION_TYPES.map((type) => {
                    ...
                    return (
                      <Pressable
                        ...
                        onPress={() => updateDraft("functionType", type)}
                      >
                        <Text
                          style={[
                            s.typeChipText,
            ...
            {canManage && (
              <View
                style={[
                  s.editCard,
                  { borderColor: "#22C55E60", marginTop: 10 },
                ]}
              >
                <View
                  style={[
                ...
                >
                  ...
                  <Text style={[s.editCardTitle, { color: "#22C55E" }]}>
                    Function Menu
                  </Text>
                  <Text
                    style={{
                ...
                >
                  ...
                  >
                    ...
                    <Text style={{ fontFamily: "Inter_600SemiBold" }}>
                      Course: Dish Name — description | Tag | Tag
                    </Text>
                    {"\n"}
                    e.g. Entrée: Tiger Prawn Cocktail — Marie Rose sauce | GF |
                    Alt: smoked salmon
                  </Text>
                </View>
                {draft.menu.map((item, idx) => (
                  ...
                  >
                    <Text
                      style={[s.fieldLabel, { paddingTop: 4, fontSize: 11 }]}
                    >
                      Course {idx + 1}
                    </Text>
                    <TextInput
            ...
            <View
              style={[s.editCard, { borderColor: tc + "60", marginTop: 10 }]}
            >
              <View
                style={[
                  s.editCardHeader,
                  { backgroundColor: tc + "15", borderBottomColor: tc + "30" },
                ]}
              >
                <Feather name="clock" size={14} color={tc} />
                <Text style={[s.editCardTitle, { color: tc }]}>
                  Service Timetable
                </Text>
                <Pressable
                  style={{
              ...
              {pasteMode ? (
                ...
                <>
                  {draft.serviceEvents.map((evt, idx) => (
                    <View
                      key={idx}
                      style={[s.courseEditRow, { paddingVertical: 8, gap: 8 }]}
                    >
                      <TextInput
                        style={[
                          s.courseEditInput,
                          { color: tc, width: 64, flexGrow: 0 },
                        ]}
                        value={evt.time}
                        onChangeText={(v) => {
                          setDraft((d) => {
                            if (!d) return d;
                            const ev = [...d.serviceEvents];
                            ev[idx] = { ...ev[idx], time: v };
                            return { ...d, serviceEvents: ev };
                          });
                          setHasUnsaved(true);
                        }}
                        placeholder="HH:MM"
                      ...
                      <Pressable
                        onPress={() => {
                          setDraft((d) => {
                            ...
                            return {
                              ...d,
                              serviceEvents: d.serviceEvents.filter(
                                (_, i) => i !== idx,
                              ),
                            };
                          });
            ...
            <View
              style={[s.editCard, { borderColor: "#F59E0B60", marginTop: 10 }]}
            >
              <View
                style={[
                  s.editCardHeader,
              ...
              >
                ...
                <Text style={[s.editCardTitle, { color: "#F59E0B" }]}>
                ...
                return (
                  <View key={idx} style={s.dietaryEditRow}>
                    <View style={s.dietaryEditNameRow}>
                      ...
                      />
                      <TextInput
                        style={[s.dietaryEditNameInput, { color: dc }]}
                        value={req.name}
                        onChangeText={(v) => updateDietaryName(idx, v)}
                    ...
                    <View style={s.dietaryEditCountRow}>
                      <Text style={s.dietaryEditCountLabel}>
                        Number of guests:
                      </Text>
                      <TextInput
                        style={s.dietaryEditCountInput}
                        value={String(req.count)}
                        onChangeText={(v) => updateDietaryCount(idx, v)}
                        keyboardType="number-pad"
                      />
                    </View>
                    <TextInput
                      style={s.dietaryEditNoteInput}
                      value={req.note ?? ""}
                      onChangeText={(v) => updateDietaryNote(idx, v)}
          ...
          <>
              ...
              >
                ...
                <Text
                  style={{
                    fontSize: 16,
                    fontFamily: "Inter_700Bold",
                    color: status === "LIVE" ? "#fff" : "#F97316",
                  }}
                >
                  {status === "LIVE"
                    ? "LIVE — Open Service Mode"
                    : "Open Service Mode"}
                </Text>
                {status === "LIVE" && (
                  <View
            ...
            {fn.chefInCharge ? (
              ...
              >
                ...
                >
                  No chef in charge set — tap Edit to assign one.
                </Text>
              </View>
            ) : null}
            ...
            >
              ...
              >
                ...
                >
                  Service Timetable — {fn.functionType}
                </Text>
              </View>
              {/* Arrive row */}
              <View
              ...
              {activeCourses.length === 0 && (
                ...
                >
                  ...
                  >
                    {canManage
                      ? "No milestones added yet — tap Edit to set the timetable."
                      : "Check with your manager for the service timetable."}
                  </Text>
                </View>
              )}
              {/* Finish row */}
                ...
                >
                  ...
                  <View
                    style={{
                      ...
                      paddingVertical: 2,
                      borderRadius: 8,
                      backgroundColor: hasSevereAllergen
                        ? "rgba(239,68,68,0.15)"
              ...
              >
                ...
                <View>
                  {fn.timeline.map((item, i) => {
                    ...
                    const isActive =
                      !isDone &&
                      i === fn.timeline.findIndex((t) => !t.completed);
                    return (
                      <Pressable
                        key={item.id}
                        style={({ pressed }) => ({
            ...
            >
                        ...
                        [
                          { text: "Cancel", style: "cancel" },
                          {
                            ...
                            onPress: () => {
                              ...
                              Alert.alert(
                                "Done",
                                "Prep tasks added to the Prep tab.",
                              );
                            },
                          },
                        ],
              ...
              {fn.menu.length === 0 ? (
                <View style={{ paddingHorizontal: 14, paddingVertical: 16 }}>
                  ...
                  >
                    {canManage
                      ? "No menu added yet — tap Edit to add dishes."
                      : "Menu not yet set. Check with your manager."}
                  </Text>
                </View>
              ) : (
                  ...
                  {fn.menu.map((line, i) => {
                    ...
                    return (
                      <View
                        ...
                        style={{
                          paddingVertical: 12,
                          borderBottomWidth: i < fn.menu.length - 1 ? 1 : 0,
                          borderBottomColor: colors.border,
                        }}
                      >
                        {course ? (
                        ...
                        {desc ? (
                          <Text
                            style={{
                              ...
                              color: colors.mutedForeground,
                              lineHeight: 19,
                              marginBottom: tags.length > 0 ? 7 : 0,
                            }}
                          >
                            {desc}
                          </Text>
            ...
            <View style={s.section}>
              ...
              {fnStaff.length === 0 && (
                ...
                >
                  ...
                  >
                    {canManage
                      ? "Add staff to this function via the Roster tab, then assign them here."
                      : "Speak to your manager for your team assignment."}
                  </Text>
                </View>
              )}
