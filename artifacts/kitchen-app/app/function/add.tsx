...
function generateTimeline(
  ...
  const setupHrs =
    guestCount > 300 ? 7 : guestCount > 150 ? 5 : guestCount > 50 ? 3 : 2;
  const kitchenOpen = fmtMins(Math.max(5 * 60, startMins - setupHrs * 60));
  add(
    kitchenOpen,
    `KITCHEN OPEN — Mise en place. All stations set, benches clear, HACCP sheets started. Cold room temp check.`,
    "setup",
  );
  if (guestCount > 40) {
    add(
      fmtMins(timeToMins(kitchenOpen) + 90),
      `Protein portioning and prep. Dietary alternates labelled and separated on dedicated trays.`,
      "setup",
    );
  }
  if (guestCount > 100) {
    add(
      fmtMins(timeToMins(kitchenOpen) + 180),
      `Sauce reductions on. Pastry items into oven. Cross-contamination check all allergen stations.`,
      "setup",
    );
  }
  if (functionType === "Buffet") {
    add(
      fmtMins(startMins - 60),
      `Chafing dishes set, water pans filled. Dietary labels placed. Carving station board and knife set.`,
      "venue",
    );
  }
  add(
    fmtMins(startMins - 40),
    `VENUE CHECK — ${room}: crockery polished, mise en place on pass, lamps on. Table numbers confirmed.`,
    "venue",
  );
  add(
    fmtMins(startMins - 15),
    `PRE-SERVICE BRIEF — All team. Runner sections confirmed. Allergen plan reviewed. Manager sign-off.`,
    "brief",
  );

  const sorted = [...serviceEvents].sort((a, b) =>
    a.time.localeCompare(b.time),
  );
  if (sorted.length > 0) {
    for (const evt of sorted) {
      const isBuffet =
        evt.label.toLowerCase().includes("open") ||
        evt.label.toLowerCase().includes("close");
    ...
    add(
      startTime,
      `SERVICE START — Guests arrive. All stations ready. ${guestCount} covers.`,
      "service",
    );
    const midMins = Math.round((startMins + endMins) / 2);
    add(
      fmtMins(midMins),
      `Mid-service check — replenish stations, clear empties. Allergen alternates confirmed.`,
      "service",
    );
  }
  add(
    fmtMins(endMins - 30),
    `Last covers cleared. Pass broken down. Leftover food labelled & chilled for HACCP.`,
    "close",
  );
  add(
    fmtMins(endMins),
    `KITCHEN CLEAR — All surfaces sanitised. HACCP sheets completed & signed by manager. Waste logged.`,
    "close",
  );
...
const TYPE_MILESTONE_TEMPLATES: Record<
  FunctionType,
  Array<{ time: string; label: string }>
> = {
  "A-la-carte": [
    { time: "", label: "Entrée Away" },
    { time: "", label: "Main Away" },
    { time: "", label: "Dessert Away" },
  ],
  "Set Menu": [
    { time: "", label: "Entrée Away" },
    { time: "", label: "Main Away" },
  ...
  Buffet: [
    { time: "", label: "Buffet Open" },
    { time: "", label: "Buffet Closed" },
  ],
  Cocktail: [
    { time: "", label: "Canapés Service" },
    { time: "", label: "Cocktail Hour Ends" },
  ],
  ...
  "High Tea": [
    { time: "", label: "High Tea Served" },
    { time: "", label: "Sandwiches Away" },
    { time: "", label: "Scones Away" },
    { time: "", label: "Sweets Away" },
  ],
};
...
export default function AddFunctionScreen() {
  ...
  const [parsedAllergens, setParsedAllergens] = useState<
    ParsedFunctionData["allergenWarnings"]
  >([]);
  const [parsedHaccp, setParsedHaccp] = useState<
    ParsedFunctionData["haccpNotes"]
  >([]);
  const [parsedTerms, setParsedTerms] = useState<
    ParsedFunctionData["detectedTerminology"]
  >([]);
  ...
  if (!isManager) {
    ...
    return (
      <View style={ns.root}>
        ...
        <Text style={ns.sub}>
          Only managers can add events. Sign in as a manager on the Roster tab
          to continue.
        </Text>
        <Pressable style={ns.btn} onPress={() => router.back()}>
          <Text style={ns.btnText}>Go back</Text>
        </Pressable>
      </View>
    );
  }
  ...
  function applyParsedData(data: ParsedFunctionData) {
    const ft: FunctionType = (FUNCTION_TYPES as string[]).includes(
      data.functionType,
    )
    ...
    setForm((prev) => ({
      ...
      guestCount:
        data.guestCount > 0 ? String(data.guestCount) : prev.guestCount,
      serviceEvents:
        data.serviceEvents.length > 0
          ? data.serviceEvents
          : prev.serviceEvents.length === 0
            ? TYPE_MILESTONE_TEMPLATES[ft]
            : prev.serviceEvents,
      dietaryRequirements:
        data.dietaryRequirements.length > 0
          ? data.dietaryRequirements.map((d) => ({
              name: d.name,
    ...
    const haccpCount =
      data.haccpNotes?.filter((n) => n.priority === "critical").length ?? 0;
    ...
    setBanner({
      ...
      prepCount: data.prepItems.length,
      aiUsed: data.aiUsed,
      allergenCount,
      haccpCount,
      termCount,
  ...
  function handleFunctionTypeChange(type: FunctionType) {
    ...
    setForm((f) => ({
      ...
      functionType: type,
      serviceEvents:
        f.serviceEvents.length === 0
          ? TYPE_MILESTONE_TEMPLATES[type]
          : f.serviceEvents,
    }));
  }
  ...
  function handleSave() {
    ...
    if (!form.name.trim()) {
      Alert.alert("Missing name", "Please enter a function name.");
      return;
    }
    if (!form.room.trim()) {
      Alert.alert("Missing room", "Please enter the room or venue.");
      return;
    }
    if (!form.startTime) {
      Alert.alert(
        "Missing start time",
        "Please enter a start time (e.g. 18:30).",
      );
      return;
    }
      ...
      <>
        <View style={s.importHero}>
          ...
          <Text style={s.importSub}>
            Paste text from your booking system, email, or any document — the AI
            reads it and fills everything in automatically.
          </Text>
        </View>
        <TextInput
          style={s.textArea}
      ...
      <>
        <View style={s.fileCard}>
          ...
          <Pressable
            style={[
              s.actionBtn,
              { marginHorizontal: 0, marginTop: 4, paddingHorizontal: 28 },
            ]}
    ...
    return (
      <>
        <View style={s.fileCard}>
          ...
          <Text style={s.fileCardText}>
            Import a photo of your function sheet
          </Text>
          <Text style={s.fileCardSub}>
  ...
  function renderAllergenWarnings() {
    ...
    const criticalCount = parsedAllergens.filter(
      (w) => w.severity === "definite",
    ).length;
    return (
      <>
        ...
        <View style={[s.card, { borderColor: "#EF444440" }]}>
          ...
          {showAllergens &&
            parsedAllergens.map((warning, idx) => {
              ...
              return (
                <View
                  key={idx}
                  style={[s.allergenItem, { borderTopColor: colors.border }]}
                >
                  <View
                    style={{
                      flexDirection: "row",
                  ...
                  >
                    ...
                    >
                      <Text style={[s.severityText, { color: col }]}>
                        {warning.severity.toUpperCase()}
                      </Text>
                    </View>
                    <Text
                      style={[s.allergenDish, { color: colors.foreground }]}
                      numberOfLines={1}
                    >
                      {dishName}
                    </Text>
                  ...
                  >
                    {warning.allergens.map((a) => (
                      ...
                      >
                        <Text style={[s.allergenTagText, { color: col }]}>
                          {a}
                        </Text>
                      </View>
                    ))}
          ...
          {showAllergens && (
            <View style={s.allergenFooter}>
              <Ionicons
                ...
                color={colors.mutedForeground}
              />
              <Text style={s.allergenFooterText}>
                Auto-inferred from dish names — always verify with client.
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={[s.card, { borderColor: "#F9741640" }]}>
          ...
          >
            ...
            >
              ...
              <Feather
                name={showHaccp ? "chevron-up" : "chevron-down"}
                size={16}
                color={colors.mutedForeground}
              />
            </View>
            ...
            (
              ...
              items.length > 0 ? (
                <View key={label}>
                  ...
                  >
                    <View
                      style={[s.haccpGroupDot, { backgroundColor: color }]}
                    />
                  ...
                  {items.map((note, idx) => (
                    <View
                      key={idx}
                      style={[s.haccpItem, { borderTopColor: colors.border }]}
                    >
                      {note.minTemp != null && (
          ...
          {showHaccp && (
            <View style={s.allergenFooter}>
              ...
              />
              <Text style={s.allergenFooterText}>
                FSANZ / MPI aligned. Complete HACCP log before service.
              </Text>
            </View>
          )}
        </View>
          ...
          {showGlossary &&
            parsedTerms.map((t, idx) => {
              ...
              return (
                <View
                  key={idx}
                  style={[s.glossaryItem, { borderTopColor: colors.border }]}
                >
                  <View
                    style={{
                      flexDirection: "row",
                  ...
                  >
                    ...
                    >
                      <Text style={[s.langBadgeText, { color: langColor }]}>
                        {t.language}
                      </Text>
                    </View>
                    <Text style={s.glossaryTerm}>{t.term}</Text>
                  </View>
                  <Text style={s.glossaryMeaning} numberOfLines={2}>
                    {t.meaning}
                  </Text>
                </View>
              );
        ...
        <View style={s.card}>
          <Pressable style={s.expandRow} onPress={() => setShowPrep((v) => !v)}>
            ...
            >
              ...
              <Feather
                name={showPrep ? "chevron-up" : "chevron-down"}
                size={16}
                color={colors.mutedForeground}
              />
            </View>
    ...
    return (
      <>
        ...
        <View style={s.card}>
          ...
          ).map(({ key, label, placeholder, keyboard }, idx, arr) => {
            const val = form[key as keyof FormState] as string;
            const isMissing =
              showMissingDots &&
              !val &&
              (key === "name" || key === "room" || key === "guestCount");
            return (
              <View
                key={key}
                style={[
            ...
            return (
              ...
              >
                ...
                <TextInput
                  ...
                  placeholderTextColor={colors.mutedForeground}
                />
                {showMissingDots && key === "startTime" && !val && (
              ...
              return (
                <Pressable
                  ...
                  style={[
                    s.typeChip,
                    {
                      backgroundColor: selected ? ftc + "25" : "transparent",
                      borderColor: selected ? ftc : colors.border,
                    },
                  ]}
                ...
                >
                  ...
                  <Text
                    style={[
                      s.typeChipText,
                      { color: selected ? ftc : colors.mutedForeground },
                    ]}
                  >
                    {type}
                  </Text>
        ...
        <View style={s.card}>
          {form.serviceEvents.map((evt, idx) => (
            <View key={idx} style={[s.fieldRow, { gap: 8 }]}>
              <TextInput
                style={[
                  s.fieldInput,
                  { width: 68, flexGrow: 0, flexShrink: 0 },
                  evt.time && s.fieldInputFilled,
                ]}
                value={evt.time}
                onChangeText={(v) =>
                  setForm((f) => ({
                    ...f,
                    serviceEvents: f.serviceEvents.map((e, i) =>
                      i === idx ? { ...e, time: v } : e,
                    ),
                  }))
                }
              ...
              <Pressable
                onPress={() =>
                  setForm((f) => ({
                    ...f,
                    serviceEvents: f.serviceEvents.filter((_, i) => i !== idx),
                  }))
                }
              >
          ...
          <Pressable
            ...
            onPress={() =>
              setForm((f) => ({
                ...f,
                serviceEvents: [...f.serviceEvents, { time: "", label: "" }],
              }))
            }
          >
            <Feather name="plus-circle" size={16} color={colors.info} />
            <Text style={[s.addDietaryText, { color: colors.info }]}>
              Add milestone
            </Text>
          </Pressable>
        </View>
              ...
              <View key={idx} style={s.dietaryRow}>
                <View style={s.dietaryInputRow}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(v) =>
                      setForm((f) => ({
                        ...f,
                        dietaryRequirements: f.dietaryRequirements.map(
                          (d, i) => (i === idx ? { ...d, name: v } : d),
                        ),
                      }))
                    }
                  ...
                  <Pressable
                    onPress={() =>
                      setForm((f) => ({
                        ...f,
                        dietaryRequirements: f.dietaryRequirements.filter(
                          (_, i) => i !== idx,
                        ),
                      }))
                    }
                ...
                <TextInput
                  ...
                  onChangeText={(v) =>
                    setForm((f) => ({
                      ...f,
                      dietaryRequirements: f.dietaryRequirements.map((d, i) =>
                        i === idx ? { ...d, note: v } : d,
                      ),
                    }))
                  }
    ...
    >
      ...
      <View style={s.tabBar}>
        <Pressable
          style={[
            s.tabBtn,
            tab === "import" && { backgroundColor: colors.primary },
          ]}
        ...
        >
          <Ionicons
            name="flash"
            size={16}
            color={tab === "import" ? "#fff" : colors.mutedForeground}
          />
          <Text
            style={[
              s.tabBtnText,
        ...
        >
          <Feather
            name="edit-3"
            size={16}
            color={tab === "manual" ? "#fff" : colors.mutedForeground}
          />
          <Text
            style={[
              s.tabBtnText,
      ...
      >
            ...
            {hasParsed && !importLoading && banner && (
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      banner.fieldsFound >= 4 ? "#22C55E15" : "#F59E0B15",
                    borderColor:
                      banner.fieldsFound >= 4 ? "#22C55E50" : "#F59E0B50",
                  },
                ]}
              >
                <Ionicons
                ...
                <View style={{ flex: 1, gap: 2 }}>
                  ...
                  {(banner.menuCount > 0 ||
                    ...
                    >
                      {[
                        banner.menuCount > 0 &&
                          `${banner.menuCount} menu items`,
                        banner.prepCount > 0 &&
                          `${banner.prepCount} prep tasks`,
                        banner.allergenCount > 0 &&
                          `${banner.allergenCount} allergen risks`,
                        banner.haccpCount > 0 &&
                          `${banner.haccpCount} critical HACCP`,
                        banner.termCount > 0 &&
                          `${banner.termCount} terms decoded`,
        ...
        {tab === "manual" && (
          <>
            ...
            >
              <Text style={[s.importTitle, { color: colors.info }]}>
                Enter function details
              </Text>
              <Text style={s.importSub}>
                Fill in what you know — you can always edit more details after
                saving.
              </Text>
            </View>
