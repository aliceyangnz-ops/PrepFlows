...
export default function ManageScreen() {
  ...
  const {
    functions, staff, prepItems, sickStaffIds, currentStaffId,
    updateStaff, deleteFunction,
  } = useKitchen();
  ...
  const s = StyleSheet.create({
    ...
    backBtn: {
      width: 36, height: 36, borderRadius: 10, backgroundColor: "#ffffff12",
      alignItems: "center", justifyContent: "center",
    },
    topTitle: { fontSize: 17, fontFamily: "Inter_700Bold", color: "#FFFFFF", flex: 1 },
    ...
    cardRow: {
      ...
      paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: colors.border, gap: 12,
    },
    cardRowLast: { borderBottomWidth: 0 },
    rowLabel: { fontSize: 14, fontFamily: "Inter_600SemiBold", color: colors.foreground, flex: 1 },
    rowSub:   { fontSize: 12, fontFamily: "Inter_400Regular", color: colors.mutedForeground },
    rowValue: { fontSize: 13, fontFamily: "Inter_700Bold", color: colors.primary },
    /* Stat grid */
    statGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10, marginBottom: 16 },
    statBox: {
      flex: 1, minWidth: 130, backgroundColor: colors.card, borderRadius: 12,
      borderWidth: 1, borderColor: colors.border, padding: 14,
    },
    statNum:   { fontSize: 28, fontFamily: "Inter_700Bold", color: colors.foreground },
  ...
  if (!isManager) {
    return (
      <View style={[s.root, s.noAccess]}>
        ...
        <Text style={{ fontSize: 18, fontFamily: "Inter_700Bold", color: colors.foreground, marginTop: 16, textAlign: "center" }}>
          Management Only
        </Text>
        <Text style={{ fontSize: 14, fontFamily: "Inter_400Regular", color: colors.mutedForeground, textAlign: "center", marginTop: 8, lineHeight: 22 }}>
        ...
        >
          ...
          <Text style={{ fontSize: 14, fontFamily: "Inter_700Bold", color: "#fff" }}>Go to Roster</Text>
        </Pressable>
      </View>
    );
  }
  ...
  const OverviewTab = () => (
    <ScrollView style={s.body} contentContainerStyle={s.pad} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.statGrid}>
        ...
        <View style={s.statBox}>
          <Text style={[s.statNum, { color: colors.accent }]}>{staff.length}</Text>
          <Text style={s.statLabel}>Staff on</Text>
        </View>
        <View style={s.statBox}>
          <Text style={[s.statNum, { color: sickStaffIds.length > 0 ? "#EF4444" : colors.foreground }]}>{sickStaffIds.length}</Text>
          <Text style={s.statLabel}>Called sick</Text>
        </View>
        <View style={s.statBox}>
          <Text style={[s.statNum, { color: "#F59E0B" }]}>{prepTotal > 0 ? Math.round((prepDone / prepTotal) * 100) : 0}%</Text>
          <Text style={s.statLabel}>Prep complete</Text>
        </View>
        <View style={s.statBox}>
          <Text style={[s.statNum, { color: "#8B5CF6" }]}>{prepDone}/{prepTotal}</Text>
      ...
      <View style={s.card}>
        {functions.map((fn, idx) => (
          <Pressable
            key={fn.id}
            style={({ pressed }) => [s.cardRow, idx === functions.length - 1 && s.cardRowLast, pressed && { opacity: 0.7 }]}
            onPress={() => router.push(`/function/${fn.id}`)}
          >
            <View style={{ flex: 1 }}>
              <Text style={s.rowLabel} numberOfLines={1}>{fn.name}</Text>
              <Text style={s.rowSub}>{fn.room} · {fn.startTime}–{fn.endTime} · {fn.guestCount} guests</Text>
            </View>
            <View style={{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, backgroundColor: colors.primary + "15" }}>
              <Text style={{ fontSize: 11, fontFamily: "Inter_700Bold", color: colors.primary }}>{fn.functionType}</Text>
            </View>
        ...
        {functions.length === 0 && (
          <View style={{ padding: 20, alignItems: "center" }}>
            <Text style={{ fontSize: 13, fontFamily: "Inter_400Regular", color: colors.mutedForeground }}>No functions today.</Text>
          </View>
        )}
      </View>
  ...
  const FunctionsTab = () => (
    <ScrollView style={s.body} contentContainerStyle={s.pad} showsVerticalScrollIndicator={false}>
      ...
      >
        ...
        <Text style={{ fontSize: 15, fontFamily: "Inter_700Bold", color: "#fff" }}>Add New Function</Text>
      </Pressable>
      ...
      {functions.map((fn) => {
        ...
        return (
          <View key={fn.id} style={[s.card, { marginBottom: 12 }]}>
            <View style={{ padding: 14, borderBottomWidth: 1, borderBottomColor: colors.border, backgroundColor: colors.primary + "08" }}>
              <Text style={{ fontSize: 16, fontFamily: "Inter_700Bold", color: colors.foreground }}>{fn.name}</Text>
              <Text style={{ fontSize: 12, fontFamily: "Inter_400Regular", color: colors.mutedForeground, marginTop: 3 }}>
                {fn.room} · {fn.floor} · {fn.startTime}–{fn.endTime} · {fn.guestCount} guests · {fn.functionType}
              </Text>
            </View>
            <View style={{ flexDirection: "row", padding: 14, gap: 10, flexWrap: "wrap" }}>
              <View style={{ flex: 1, minWidth: 80 }}>
                <Text style={{ fontSize: 11, fontFamily: "Inter_400Regular", color: colors.mutedForeground }}>Staff assigned</Text>
                <Text style={{ fontSize: 18, fontFamily: "Inter_700Bold", color: colors.foreground }}>{fn.teamIds.length}</Text>
              </View>
              <View style={{ flex: 1, minWidth: 80 }}>
                <Text style={{ fontSize: 11, fontFamily: "Inter_400Regular", color: colors.mutedForeground }}>Dietary</Text>
                <Text style={{ fontSize: 18, fontFamily: "Inter_700Bold", color: fn.dietaryRequirements.length > 0 ? "#F59E0B" : colors.foreground }}>
                  {fn.dietaryRequirements.reduce((n, r) => n + r.count, 0)}
                </Text>
              </View>
              <View style={{ flex: 1, minWidth: 80 }}>
                <Text style={{ fontSize: 11, fontFamily: "Inter_400Regular", color: colors.mutedForeground }}>Prep done</Text>
            ...
            <View style={{ flexDirection: "row", gap: 10, paddingHorizontal: 14, paddingBottom: 14 }}>
              <Pressable
                style={({ pressed }) => [s.actionBtn, { flex: 1, justifyContent: "center", borderColor: colors.primary + "50", backgroundColor: colors.primary + "10", opacity: pressed ? 0.7 : 1 }]}
                onPress={() => router.push(`/function/${fn.id}`)}
              >
                <Feather name="edit-3" size={14} color={colors.primary} />
                <Text style={[s.actionBtnText, { color: colors.primary }]}>Open & Edit</Text>
              </Pressable>
              <Pressable
                style={({ pressed }) => [s.actionBtn, { borderColor: "#EF444450", backgroundColor: "#EF444410", opacity: pressed ? 0.7 : 1, paddingHorizontal: 14 }]}
                onPress={() => {
                  Alert.alert("Delete Function", `Delete "${fn.name}"? This cannot be undone.`, [
                    { text: "Cancel", style: "cancel" },
                    { text: "Delete", style: "destructive", onPress: () => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy); deleteFunction(fn.id); } },
                  ]);
                }}
              >
      ...
      {functions.length === 0 && (
        <View style={[s.card, { padding: 24, alignItems: "center" }]}>
          ...
          <Text style={{ fontSize: 14, fontFamily: "Inter_500Medium", color: colors.mutedForeground, marginTop: 12 }}>No functions scheduled.</Text>
        </View>
      )}
      <View style={{ height: Platform.OS === "web" ? 40 : insets.bottom + 40 }} />
    </ScrollView>
  ...
  const RosterTab = () => (
    <ScrollView style={s.body} contentContainerStyle={s.pad} showsVerticalScrollIndicator={false}>
      ...
      >
        ...
        <Text style={{ fontSize: 15, fontFamily: "Inter_700Bold", color: "#fff" }}>Add Staff Member</Text>
      </Pressable>
      ...
      {staff.map((member) => {
        ...
        return (
          <Pressable
            key={member.id}
            style={({ pressed }) => [s.cardRow, s.card, { marginBottom: 8, opacity: pressed ? 0.8 : 1 }]}
          ...
          >
            <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: tc + "20", alignItems: "center", justifyContent: "center" }}>
              <Text style={{ fontSize: 15, fontFamily: "Inter_700Bold", color: tc }}>{member.name.charAt(0)}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                <Text style={s.rowLabel} numberOfLines={1}>{member.name}</Text>
                {isSick && <View style={{ paddingHorizontal: 6, paddingVertical: 2, backgroundColor: "#EF444420", borderRadius: 5 }}><Text style={{ fontSize: 10, fontFamily: "Inter_700Bold", color: "#EF4444" }}>SICK</Text></View>}
              </View>
              <Text style={s.rowSub}>{member.role} · {member.staffNumber} · {member.shiftStart}–{member.shiftEnd}</Text>
            </View>
            <View style={[s.accessPill, { backgroundColor: tc + "20" }]}>
              <Text style={[s.accessPillText, { color: tc }]}>{level === "manager" ? "MGMT" : level === "team_leader" ? "LEAD" : "STAFF"}</Text>
            </View>
      ...
      {staff.map((member) => {
        ...
        return (
          <View key={member.id} style={[s.card, { marginBottom: 10 }]}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 12, padding: 14, borderBottomWidth: 1, borderBottomColor: colors.border }}>
              <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: tc + "20", alignItems: "center", justifyContent: "center" }}>
                <Text style={{ fontSize: 15, fontFamily: "Inter_700Bold", color: tc }}>{member.name.charAt(0)}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 15, fontFamily: "Inter_700Bold", color: colors.foreground }}>{member.name}</Text>
                <Text style={{ fontSize: 12, fontFamily: "Inter_400Regular", color: colors.mutedForeground }}>{member.role} · {member.staffNumber}</Text>
              </View>
              <View style={[s.accessPill, { backgroundColor: tc + "20" }]}>
                <Text style={[s.accessPillText, { color: tc }]}>{ACCESS_LEVEL_LABELS[current]}</Text>
              </View>
            </View>
            <View style={{ flexDirection: "row", gap: 8, padding: 12 }}>
              {ACCESS_OPTIONS.map((level) => {
                ...
                return (
                  ...
                  >
                    <Text style={{ fontSize: 11, fontFamily: isActive ? "Inter_700Bold" : "Inter_400Regular", color: isActive ? lc : colors.mutedForeground }}>
                      {level === "manager" ? "Manager" : level === "team_leader" ? "Team Lead" : "Staff"}
                    </Text>
                  </Pressable>
                );
              })}
      ...
      <View style={s.topBar}>
        <View style={s.topBarInner}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {currentMember && (
              <Text style={s.topSub}>{currentMember.name} · {currentMember.role}</Text>
            )}
          </View>
        ...
        {/* Tab bar */}
        <View style={s.tabBar}>
          {(["overview","functions","roster","access"] as ManageTab[]).map((t) => (
            <Pressable
              key={t}
            ...
            >
              <Text style={[s.tabText, tab === t && s.tabTextActive]}>
                {t === "overview" ? "Overview" : t === "functions" ? "Functions" : t === "roster" ? "Roster" : "Access"}
              </Text>
            </Pressable>
          ))}
        </View>
