...
function getTeamDescription(team: PrepTeam): string {
  switch (team) {
    case "Cold Larder":
      return "Cold starters, salads, sauces, canapé prep";
    case "Butchery":
      return "Meat portioning, trimming & protein prep";
    case "Hot Kitchen":
      return "Hot mains, sauces, firing & cooking";
    case "Pastry":
      return "Desserts, pastry & baked goods";
    case "Function Team":
      return "Room setup, food assembly & service running";
    default:
      return "";
  }
}
...
export default function PrepPrintScreen() {
  ...
  const printedAt = new Date().toLocaleTimeString("en-AU", {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  if (!fn) {
    return (
      ...
      >
        <Text style={{ color: "#111", fontFamily: "Inter_600SemiBold" }}>
          Event not found
        </Text>
      </View>
    );
  ...
  async function handleShare() {
    ...
    if (section !== "runsheet") {
      ...
      TEAM_ORDER.forEach((team) => {
        ...
        lines.push(`\n═ ${team.toUpperCase()} ═`);
        if (leader)
          lines.push(`  Team Leader: ${leader.name} (${leader.role})`);
        items.forEach((item) => {
          const done = item.completed ? "[✓]" : "[ ]";
          lines.push(`${done} ${item.dish}`);
          lines.push(
            `    Qty: ${item.quantity}  |  Ready by: ${item.deadline}`,
          );
      ...
      <View style={s.dietarySection}>
        <View
          style={[
            s.sectionDivider,
            {
              backgroundColor: hasSevere ? "#FEF2F2" : "#FFFBEB",
              borderColor: hasSevere ? "#FECACA" : "#FDE68A",
            },
          ]}
        >
          <Ionicons
            name="warning"
            size={14}
            color={hasSevere ? "#DC2626" : "#D97706"}
          />
          <Text
            style={[
              s.sectionDividerText,
              { color: hasSevere ? "#DC2626" : "#D97706", flex: 1 },
            ]}
          >
          ...
          <View
            style={[
              s.sectionCount,
              { backgroundColor: hasSevere ? "#FEE2E2" : "#FEF3C7" },
            ]}
          >
            <Text
              style={[
        ...
        {hasSevere && (
          <View style={s.severeWarning}>
            ...
            <Text style={s.severeText}>
              SEVERE ALLERGEN ON THIS FUNCTION — Confirm alternates before
              service. Epinephrine on site.
            </Text>
          </View>
        )}
          ...
          {dietaryReqs.map((req, idx) => {
            ...
            return (
              <View
                ...
                style={[
                  s.dietaryBadge,
                  { backgroundColor: bg, borderColor: dc + "50" },
                ]}
              >
                ...
                <View>
                  ...
                  {req.note ? (
                    <Text style={[s.dietaryNote, { color: dc }]}>
                      {req.note}
                    </Text>
                  ) : null}
                </View>
    ...
    return (
      <View style={s.runSheetSection}>
        <View style={[s.sectionDivider, { backgroundColor: "#F8FAFC" }]}>
          ...
          <Text style={[s.sectionDividerText, { color: "#1E293B", flex: 1 }]}>
            Service Run Sheet
          </Text>
        ...
        {timelineGrouped.map(({ cat, items }) => {
          ...
          return (
            <View
              ...
              style={[
                s.catGroup,
                { borderLeftWidth: 4, borderLeftColor: cs.borderLeft },
              ]}
            >
              <View style={[s.catHeader, { backgroundColor: cs.bgColor }]}>
                <View
                  style={{
                    ...
                    paddingVertical: 3,
                    borderRadius: 5,
                    backgroundColor: cs.borderLeft + "30",
                  }}
                >
                  <Text style={[s.catLabel, { color: cs.color }]}>
                    {cs.label}
                  </Text>
                </View>
                <Text style={s.catCount}>
                  {catDone}/{items.length} complete
                </Text>
              </View>
              {items.map((item) => (
                <View
                  ...
                  style={[
                    s.timelineRow,
                    { backgroundColor: item.completed ? "#F8FAFC" : "#fff" },
                  ]}
                >
                  ...
                  <View
                    style={[
                      s.timelineCheck,
                      item.completed && s.timelineCheckDone,
                    ]}
                  >
                    {item.completed && (
            ...
            <View key={team} style={s.teamBlock}>
              <View style={[s.teamTitleRow, { backgroundColor: tc + "0A" }]}>
                ...
                <View
                  style={[
                    s.teamBadge,
                    { backgroundColor: allDone ? "#F0FDF4" : "#FEF2F2" },
                  ]}
                >
                  <Text
                    style={[
              ...
              {leader && (
                <View
                  style={[
                    s.leaderRow,
                    { backgroundColor: tc + "07", borderTopColor: tc + "25" },
                  ]}
                >
                  ...
                  <View style={s.leaderInfo}>
                    <Text style={[s.leaderRole, { color: tc }]}>
                      Team Leader
                    </Text>
                ...
                <View style={[s.membersRow, { borderTopColor: tc + "20" }]}>
                  ...
                  {members.map((m) => (
                    <View
                      ...
                      style={[
                        s.memberTag,
                        { backgroundColor: tc + "12", borderColor: tc + "35" },
                      ]}
                    >
                      <Text style={[s.memberTagText, { color: tc }]}>
                        {m.name}
                      </Text>
                    </View>
                  ))}
                ...
                <View key={item.id} style={s.prepItemRow}>
                  ...
                  <View style={s.itemContent}>
                    <View style={s.itemTopRow}>
                      <Text style={[s.dishName, item.completed && s.dishDone]}>
                        {item.dish}
                      </Text>
                      <View
                        style={[
                          s.deadlineBox,
                          item.completed && s.deadlineBoxDone,
                        ]}
                      >
                        ...
                        <Text
                          style={[
                            s.deadlineTime,
                            item.completed && s.deadlineTimeDone,
                          ]}
                        >
            ...
            <View style={s.docStatusRow}>
              ...
              {totalDietary > 0 && (
                <View
                  style={[
                    s.docStatusBox,
                    hasSevere && { backgroundColor: "rgba(239,68,68,0.2)" },
                  ]}
                >
                  <Text
                    style={[s.docStatusNum, hasSevere && { color: "#FCA5A5" }]}
                  >
                    {totalDietary}
                  </Text>
                  <Text
          ...
          <View style={s.footer}>
            <View style={s.footerLeft}>
              ...
              <Text style={s.footerText}>
                {fn.name} · {fn.room} · {fn.guestCount} covers
              </Text>
              {totalDietary > 0 && (
                <Text
                  style={[
                ...
                >
                  {hasSevere
                    ? "⚠ SEVERE ALLERGEN — check dietary notes"
                    : `${totalDietary} dietary guests — see requirements above`}
                </Text>
              )}
            </View>
