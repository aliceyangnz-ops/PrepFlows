...
function roomToForm(room: KitchenRoom): RoomForm {
  return {
    ...
    cocktailCapacity: room.cocktailCapacity
      ? String(room.cocktailCapacity)
      : "",
...
export default function RoomsScreen() {
  ...
  const { rooms, addRoom, updateRoom, removeRoom, currentStaffId, staff } =
    useKitchen();
  ...
  function handleSave() {
    if (!form.name.trim()) {
      Alert.alert("Missing room name", "Please enter a name for this room.");
      return;
    }
    const cap = parseInt(form.capacity, 10);
    if (isNaN(cap) || cap < 1) {
      Alert.alert("Invalid capacity", "Please enter a valid maximum capacity.");
      return;
    }
    ...
    const roomData: Omit<KitchenRoom, "id"> = {
      ...
      style: form.style,
      capacity: cap,
      banquetCapacity: form.banquetCapacity
        ? parseInt(form.banquetCapacity, 10) || undefined
        : undefined,
      cocktailCapacity: form.cocktailCapacity
        ? parseInt(form.cocktailCapacity, 10) || undefined
        : undefined,
      theatreCapacity: form.theatreCapacity
        ? parseInt(form.theatreCapacity, 10) || undefined
        : undefined,
  ...
  function renderRoomCard(room: KitchenRoom) {
    ...
    return (
      <View
        key={room.id}
        style={[s.roomCard, !room.isActive && s.roomCardInactive]}
      >
        <Pressable
          style={s.roomHeader}
          onPress={() => setExpandedId(isOpen ? null : room.id)}
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.roomFloor}>
              {room.floor} · {room.style}
            </Text>
          </View>
          {!room.isActive && (
        ...
        >
          ...
          >
                  ...
                  return (
                    <Pressable
                      ...
                      style={[
                        s.chip,
                        {
                          backgroundColor: selected ? sc + "25" : "transparent",
            ...
            <Text style={s.formSection}>
              ...
              >
                — catering access, setup rules, etc.
              </Text>
            </Text>
            <TextInput
              style={s.notesInput}
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.summaryRow}>
        ...
        <View style={s.summaryCard}>
          <Text style={[s.summaryNum, { fontSize: 16 }]}>
      ...
      >
        ...
        <View
          style={{ height: Platform.OS === "web" ? 80 : insets.bottom + 80 }}
        />
      </ScrollView>
