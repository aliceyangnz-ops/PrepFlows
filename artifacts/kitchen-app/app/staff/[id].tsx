...
const ACCESS_OVERRIDES: Array<{
  ...
  {
    value: "manager",
    label: "Manager / Head Office",
    desc: "Full edit — functions, menu, roster, access",
  },
  {
    value: "team_leader",
    label: "Team Leader",
    desc: "Read all, hide own functions, check prep",
  },
  {
    value: "staff",
    label: "Staff / Casual",
    desc: "Read only — assigned functions only",
  },
];
...
export default function StaffEditScreen() {
  ...
  const { staff, addStaff, updateStaff, removeStaff, currentStaffId } =
    useKitchen();
  const currentMember = currentStaffId
    ? (staff.find((m) => m.id === currentStaffId) ?? null)
    : null;
  ...
  const [section, setSection] = useState<PrepTeam | undefined>(
    existing?.section,
  );
  const [isTeamLead, setIsTeamLead] = useState(!!existing?.teamLeadFor);
  const [accessOverride, setAccessOverride] = useState<AccessLevel | undefined>(
  ...
  function handleSave() {
    if (!isManager && !isBootstrap) {
      Alert.alert(
        "Access denied",
        "Only managers can add or edit staff members.",
      );
      return;
    }
    if (!name.trim()) {
  ...
  if (!isManager && !isBootstrap) {
    return (
      ...
      >
        ...
        >
          Only managers can add or edit staff members. Sign in as a manager on
          the Roster tab to continue.
        </Text>
        <Pressable
          style={{
            marginTop: 24,
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={s.hero}>
          ...
          <Text style={s.heroSub}>
            {isNew
              ? "Fill in their details — they can select themselves on the Roster tab"
        ...
        <View style={s.body}>
          ...
          <View style={[s.card, { borderColor: colors.primary + "60" }]}>
            ...
            >
              ...
              <Text style={[s.cardTitle, { color: colors.primary }]}>
                Personal Details
              </Text>
            </View>
          ...
          >
            The PIN lets this person sign in on the Roster screen. Use their
            phone number (last 4 digits) as a backup if PIN is not set. PINs are
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      s.chip,
                      {
                        backgroundColor: selected ? rc + "25" : "transparent",
                        borderColor: selected ? rc : colors.border,
                      },
                    ]}
                    onPress={() => {
                      setRole(r);
                  ...
                  >
                    <Text
                      style={[
                        s.chipText,
                        { color: selected ? rc : colors.mutedForeground },
                      ]}
                    >
                      {r}
                    </Text>
          ...
          <View style={[s.card, { borderColor: "#3B82F660" }]}>
            ...
            >
              ...
              <Text style={[s.cardTitle, { color: "#3B82F6" }]}>
                Shift Times
              </Text>
              <Text
                style={{
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      s.chip,
                      {
                        backgroundColor: selected ? tc + "25" : "transparent",
                  ...
                  >
                    <Text
                      style={[
                        s.chipText,
                        { color: selected ? tc : colors.mutedForeground },
                      ]}
                    >
                      {t}
                    </Text>
            ...
            {section && (
              ...
              >
                <Pressable
                  style={[
                    s.toggleTrack,
                    { backgroundColor: isTeamLead ? "#22C55E" : colors.border },
                  ]}
                  onPress={() => {
                ...
                >
                  <View
                    style={[
                      s.toggleThumb,
                      { alignSelf: isTeamLead ? "flex-end" : "flex-start" },
                    ]}
                  />
                </Pressable>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.toggleSub}>
                    Will appear as the contact on the casual staff QR brief
                  </Text>
                </View>
              </View>
            )}
          ...
          <View
            style={[s.card, { borderColor: colors.border, marginBottom: 6 }]}
          >
            <View
              style={[
                s.cardHeader,
            ...
            <View style={s.accessRow}>
              ...
              >
                Leave unset to use role default (e.g. Head Chef → Manager,
                Casual → Staff)
              </Text>
              {ACCESS_OVERRIDES.map((opt) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => {
                      setAccessOverride(selected ? undefined : opt.value);
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    }}
                  >
                    <Ionicons
                      name={selected ? "shield-checkmark" : "shield-outline"}
                      size={18}
