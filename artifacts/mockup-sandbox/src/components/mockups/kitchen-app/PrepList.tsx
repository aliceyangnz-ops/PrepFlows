...
function PrepItem({
  ...
  return (
    ...
    >
      <div
        className={`mt-1 mr-3 flex-shrink-0 w-6 h-6 rounded flex items-center justify-center border ${isDone ? "bg-emerald-500 border-emerald-500 text-white" : "bg-zinc-950 border-zinc-600 text-transparent"}`}
      >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start mb-1">
          <h4
            className={`font-semibold text-sm truncate ${isDone ? "line-through text-zinc-500" : "text-zinc-100"}`}
          >
            {title}
          </h4>
          <span
            className={`text-xs font-bold whitespace-nowrap ml-2 ${isDone ? "text-zinc-600" : "text-amber-500"}`}
          >
            {qty}
          </span>
        </div>
