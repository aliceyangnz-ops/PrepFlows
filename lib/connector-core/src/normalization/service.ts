...
const DATE_PATTERNS: Array<{ re: RegExp; parse: (m: RegExpMatchArray) => string }> = [
  ...
  { re: /^(\d{1,2})\/(\d{1,2})\/(\d{4})/, parse: (m) => {
    ...
    if (mo > 12) return `${m[3]}-${m[2]!.padStart(2,"0")}-${m[1]!.padStart(2,"0")}`;
  ...
  { re: /^([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})/, parse: (m) => {
...
type KitchenFunctionType =
  | "A-la-carte" | "Buffet" | "Cocktail" | "Canapés"
  | "Canapés + A-la-carte" | "School Ball" | "Set Menu" | "High Tea";
...
const EVENT_TYPE_MAP: Array<{ patterns: RegExp; value: KitchenFunctionType }> = [
  ...
  { patterns: /canap[eé]s?\s*\+?\s*a.?la.?carte/i,                  value: "Canapés + A-la-carte" },
  { patterns: /canap[eé]s?/i,                                        value: "Canapés" },
  { patterns: /school\s*ball|formal|prom/i,                          value: "School Ball" },
  { patterns: /set\s*menu|table\s*d.?h[oô]te|prix\s*fixe/i,         value: "Set Menu" },
  { patterns: /high\s*tea|afternoon\s*tea/i,                         value: "High Tea" },
  { patterns: /a.?la.?carte|plated|sit.?down|seated\s*dinner/i,      value: "A-la-carte" },
];
...
export function parseDietaryRequirements(raw: string): UnifiedDietaryRequirement[] {
