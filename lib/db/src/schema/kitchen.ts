import { pgTable, text, integer, boolean, jsonb, timestamp, uuid, pgEnum } from "drizzle-orm/pg-core";
import { createInsertSchema, createSelectSchema } from "drizzle-zod";
import { z } from "zod/v4";

export const functionStatusEnum = pgEnum("function_status", ["upcoming", "active", "completed"]);
export const functionTypeEnum = pgEnum("function_type", [
  "A-la-carte", "Buffet", "Cocktail", "Canapés", "Canapés + A-la-carte",
  "School Ball", "Set Menu", "High Tea",
]);
...
export const kitchenFunctionsTable = pgTable("kitchen_functions", {
  ...
  functionType: functionTypeEnum("function_type").notNull().default("A-la-carte"),
  date: text("date"),
  ...
  dietaryRequirements: jsonb("dietary_requirements").$type<Array<{ name: string; count: number; note?: string }>>().notNull().default([]),
  serviceTimes: jsonb("service_times").$type<Record<string, string>>(),
  serviceEvents: jsonb("service_events").$type<Array<{ time: string; label: string }>>(),
  teamIds: jsonb("team_ids").$type<string[]>().notNull().default([]),
  timeline: jsonb("timeline").$type<Array<{ id: string; time: string; task: string; category: string; completed: boolean }>>().notNull().default([]),
...
export const importJobsTable = pgTable("import_jobs", {
  ...
  errors: jsonb("errors").$type<Array<{ row: number; field: string; message: string }>>().notNull().default([]),
  warnings: jsonb("warnings").$type<Array<{ row: number; field: string; message: string }>>().notNull().default([]),
  rawPreview: jsonb("raw_preview").$type<Record<string, unknown>[]>(),
});

export const insertKitchenFunctionSchema = createInsertSchema(kitchenFunctionsTable);
export const selectKitchenFunctionSchema = createSelectSchema(kitchenFunctionsTable);
export type KitchenFunctionRow = typeof kitchenFunctionsTable.$inferSelect;
export type InsertKitchenFunctionRow = typeof kitchenFunctionsTable.$inferInsert;
