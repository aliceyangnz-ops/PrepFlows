...
export const prepItemsTable = pgTable("prep_items", {
  ...
  functionId: text("function_id")
    .notNull()
    .references(() => kitchenFunctionsTable.id, { onDelete: "cascade" }),
...
export const insertBroadcastMessageSchema = createInsertSchema(
  broadcastMessagesTable,
);
export const selectBroadcastMessageSchema = createSelectSchema(
  broadcastMessagesTable,
);
export type BroadcastMessageRow = typeof broadcastMessagesTable.$inferSelect;
export type InsertBroadcastMessageRow =
  typeof broadcastMessagesTable.$inferInsert;
