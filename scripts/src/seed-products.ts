  ...
  try {
    ...
    if (existingPro.data.length > 0) {
      ...
      proProduct = await stripe.products.create({
        name: 'PrepFlows Pro',
        description: 'Full operational system for active kitchens. Unlimited functions, prep lists, run sheets, casual staff QR briefs, up to 30 staff members.',
    ...
    if (existingTeam.data.length > 0) {
      ...
      const teamProduct = await stripe.products.create({
        name: 'PrepFlows Team',
        description: 'Multi-event coordination and full staff management. Everything in Pro plus multiple simultaneous events, broadcast messages, unlimited staff, priority support.',
      ...
      console.log(`  Yearly price:  $1,910.40/yr  (${teamYearly.id})  (save 20%)`);
    }
