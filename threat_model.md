...
- **Staff data** — staff names, staff numbers, phone numbers, role labels, team assignments, and shift times. This is personal data and also drives in-app privilege decisions.
...
- **Highest-risk code areas**: `artifacts/kitchen-app/context/KitchenContext.tsx`, `artifacts/kitchen-app/app/(tabs)/roster.tsx`, `artifacts/kitchen-app/app/staff/[id].tsx`, `artifacts/kitchen-app/app/function/[id].tsx`, `artifacts/kitchen-app/app/function/add.tsx`, `artifacts/kitchen-app/app/brief/[id].tsx`
- **Public vs authenticated/admin surfaces**: there is no real authentication boundary today; role/identity is derived from locally selected staff and optional access overrides
...
Privilege is derived from mutable client-side data (`currentStaffId`, role, and access-level override) and currently appears to control access to edit functions, roster management, and broadcast capabilities. The system must enforce privilege server-side or through another trusted mechanism; client-side route access and local state alone are not sufficient security controls for privileged actions.
